...
def get_face_descriptor(img_url):
    """Baixa imagem e extrai descritor facial. Retorna None se não achar rosto."""
    try:
...
def processar_album():
    """
    Fotógrafo ativa album → chama este endpoint → processa todas as fotos em background.
    Body: { "album_key": "bdpKrr", "evento_id": "uuid" }
    """
    data  = request.json
    ...
    return jsonify({'processadas': len(resultados), 'total': total, 'com_rosto': len(resultados)})

@app.route('/buscar', methods=['POST'])
def buscar():
    """
    Aluno envia descritor do rosto → retorna fotos que batem.
    Body: { "descriptor": [...128 floats...], "album_key": "bdpKrr", "threshold": 0.52 }
    """
    data       = request.json
    ...
    if desc_aluno.shape != (128,):
        return jsonify({'erro': 'descriptor invalido — precisa de 128 valores'}), 400
    if not album_key or not sb:
        return jsonify({'erro': 'album_key ou Supabase nao configurado'}), 400

    # Busca descritores salvos deste album
    ...
    for row in rows:
        ...
        dist = np.linalg.norm(desc_aluno - desc_foto)
        if dist < threshold:
            matched.append({
                'image_key': row['image_key'],
                'filename':  row['filename'],
                'thumb':     row['thumb_url'],
                'original':  row['web_url'],
...
def status_album():
    """Verifica quantas fotos foram processadas de um album."""
